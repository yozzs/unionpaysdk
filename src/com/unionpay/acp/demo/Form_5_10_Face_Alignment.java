 ...
 */
public class Form_5_10_Face_Alignment extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		data.put("bizType", "000803");                         //业务类型 人脸比对

		data.put("productId", "S001");                         //产品编码 人脸比对
		...
		String facePicturePath="C:\\work\\Koala.jpg"; //图片路径
		data.put("facePicture", AcpService.enImgContent(facePicturePath,DemoBase.encoding));  //M  人脸图片1  100k以内
		
		//data.put("facePictureOther", "");  // C人脸图片2 后台交易时必送 变长，照片大小要求在100K之内，jpg格式。对图片进行base64编码
		
	   //data.put("pictureToken ", "");  //C 图片标识码  前台（已经过SDK活检过）交易必送
			...
			if(AcpService.validate(rspData, DemoBase.encoding)){
				...
				if(("00").equals(rspData.get("respCode"))){//如果查询交易成功
					...
					if(("00").equals(origRespCode)){
						...
						//订单处理中或交易状态未明，需稍后发起人脸比对 【如果最终尚未确定交易是否成功请以对账文件为准】
						//TODO
					}else{
						//其他应答码为交易失败
						//TODO
		...
		resp.getWriter().write("人脸比对</br>请求报文:<br/>"+reqMessage+"<br/>" + "应答报文:</br>"+rspMessage+"");
