 ...
 */
public class Form_5_18_SMS_Code_Verfication extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		data.put("txnType", "86");                             //交易类型 00-默认
		data.put("txnSubType", "02");                          //交易子类型  默认00
		data.put("bizType", "000803");                         //业务类型 代收
		
		data.put("productId", "D002");                         //产品编码 
		
		/***商户接入参数***/
		data.put("merId", merId);                  			   //商户号码，请改成自己申请的商户号或者open上注册得来的777商户号测试改
		
		/***要调通交易以下字段必须修改***/
		data.put("orderId", orderId);                 //****商户订单号，每次发交易测试需修改为被查询的交易的订单号
		data.put("txnTime", txnTime);                 //****订单发送时间，每次发交易测试需修改为被查询的交易的订单发送时间
		
		//银行卡验证信息及身份信息 仅上送手机号码、短信验证码
			...
			if(AcpService.validate(rspData, DemoBase.encoding)){
				...
				if(("00").equals(rspData.get("respCode"))){//如果查询交易成功
					...
					if(("00").equals(origRespCode)){
						...
						//订单处理中或交易状态未明，需稍后发起短信验证码验证交易 【如果最终尚未确定交易是否成功请以对账文件为准】
						//TODO
					}else{
						//其他应答码为交易失败
						//TODO
					...
					//订单不存在，可认为交易状态未明，需要稍后发起短信验证码验证，或依据对账结果为准
		...
		resp.getWriter().write("短信验证码验证交易</br>请求报文:<br/>"+reqMessage+"<br/>" + "应答报文:</br>"+rspMessage+"");
