 ...
 */
public class Form_5_13_ID_Photo_Check extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		data.put("productId", "S024");     //   产品编码 身份证核查及证件照比                 
		...
		String facePicturePath="";//"C:\\work\\Koala.jpg"; //图片路径    //照片大小要求在400K之内，jpg格式，对图片进行base64编码。
		data.put("facePicture", AcpService.enImgContent(facePicturePath,DemoBase.encoding));  //后台交易必送。
		 
		//data.put("pictureToken", "");   //图片标识码 前台（已经过SDK活检过）交易必送。
			...
			if(AcpService.validate(rspData, DemoBase.encoding)){
				...
				if(("00").equals(rspData.get("respCode"))){//如果查询交易成功
					...
					if(("00").equals(origRespCode)){
						...
						//订单处理中或交易状态未明，需稍后发起身份证核查及证件照比对交易 【如果最终尚未确定交易是否成功请以对账文件为准】
						//TODO
					}else{
						//其他应答码为交易失败
						//TODO
					...
					//订单不存在，可认为交易状态未明，需要稍后发起身份证核查及证件照比对，或依据对账结果为准
		...
		resp.getWriter().write("身份证核查及证件照比对交易</br>请求报文:<br/>"+reqMessage+"<br/>" + "应答报文:</br>"+rspMessage+"");
