 ...
 */
public class Form_5_19_Bioassay_In_Order extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		data.put("appId", "");  //iOS上送：Bundle ID；	Android上送：包名
	//	data.put("appSign", "");  //Android必送。Android上送：包签名
		
		/**请求参数设置完毕，以下对请求参数进行签名并发送http post请求，接收同步应答报文------------->**/
		
		Map<String, String> reqData = AcpService.sign(data,DemoBase.encoding);			//报文中certId,signature的值是在signData方法中获取并自动赋值的，只要证书配置正确即可。
		String url = SDKConfig.getConfig().getAppRequestUrl();								//交易请求url从配置文件读取对应属性文件acp_sdk.properties中的 acpsdk.singleQueryUrl
			...
			if(AcpService.validate(rspData, DemoBase.encoding)){
				...
				if(("00").equals(rspData.get("respCode"))){//如果查询交易成功
					...
					if(("00").equals(origRespCode)){
						...
						//订单处理中或交易状态未明，需稍后发起活体检测下单交易 【如果最终尚未确定交易是否成功请以对账文件为准】
						//TODO
					}else{
						//其他应答码为交易失败
						//TODO
					...
					//订单不存在，可认为交易状态未明，需要稍后发起活体检测下单，或依据对账结果为准
		...
		resp.getWriter().write("活体检测下单交易</br>请求报文:<br/>"+reqMessage+"<br/>" + "应答报文:</br>"+rspMessage+"");
