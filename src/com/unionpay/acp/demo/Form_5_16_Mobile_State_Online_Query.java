 ...
 */
public class Form_5_16_Mobile_State_Online_Query extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		data.put("txnType", "87");                             //交易类型 
		data.put("txnSubType", "02");                          //交易子类型
		data.put("bizType", "000803");                         //业务类型
		
		data.put("productId", "Y004");                         //产品编码 
			...
			if(AcpService.validate(rspData, DemoBase.encoding)){
				...
				if(("00").equals(rspData.get("respCode"))){//如果查询交易成功
					...
					if(("00").equals(origRespCode)){
						...
						//订单处理中或交易状态未明，需稍后发起移动运营商手机状态和在网时长查询交易 【如果最终尚未确定交易是否成功请以对账文件为准】
						//TODO
					}else{
						//其他应答码为交易失败
						//TODO
					...
					//订单不存在，可认为交易状态未明，需要稍后发起移动运营商手机状态和在网时长查询，或依据对账结果为准
		...
		resp.getWriter().write("移动运营商手机状态和在网时长查询交易</br>请求报文:<br/>"+reqMessage+"<br/>" + "应答报文:</br>"+rspMessage+"");
