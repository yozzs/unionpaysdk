 ...
 */
public class Form_5_8_Enterprise_Info_Batch_Identification extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		data.put("txnType", "21");                             //交易类型 
		data.put("txnSubType", "04");                          //交易子类型： 企业信息验证
		data.put("bizType", "000803");                         //产品类型 

		data.put("productId", "G017");                         //产品编码：工商-企业信息批量认证
		...
		data.put("totalQty", totalQty);  //M  总笔数
		
		 String filePath="";//填写文件路径
		data.put("fileContent", AcpService.enCodeFileContent(filePath, DemoBase.encoding));  //使用DEFLATE压缩算法压缩后，Base64编码的方式传输经压缩编码的文件内容。具体格式参考《平台接口规范-第3部分-文件接口》第4章TXT格式
		...
		String url = SDKConfig.getConfig().getBatchTransUrl();								//交易请求url从配置文件读取对应属性文件acp_sdk.properties中的 acpsdk.singleQueryUrl
			...
			if(AcpService.validate(rspData, DemoBase.encoding)){
				...
				if(("00").equals(rspData.get("respCode"))){//如果查询交易成功
					...
					if(("00").equals(origRespCode)){
						...
						//订单处理中或交易状态未明，需稍后发起企业信息批量认证交易 【如果最终尚未确定交易是否成功请以对账文件为准】
						//TODO
					}else{
						//其他应答码为交易失败
						//TODO
					...
					//订单不存在，可认为交易状态未明，需要稍后发起企业信息批量认证，或依据对账结果为准
		...
		resp.getWriter().write("企业信息批量认证交易</br>请求报文:<br/>"+reqMessage+"<br/>" + "应答报文:</br>"+rspMessage+"");
