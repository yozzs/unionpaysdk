 ...
 */
public class Form_5_20_Bank_Card_Identification_Front extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		data.put("txnSubType", "12");                          //交易子类型  默认00
		data.put("bizType", "000803");                         //业务类型 
		
		//B136：银行卡二要素（卡号、姓名）
		//B 144：银行卡二要素（卡号、证件类型、证件号）
		//B 132：银行卡二要素（卡号、手机号）
		//B152：银行卡三要素（卡号、身份证号、姓名）
		//B156：银行卡四要素（卡号、手机号、身份证号、姓名）
		data.put("productId", "B136");                         //产品编码
		...
		String url = SDKConfig.getConfig().getFrontRequestUrl();									//交易请求url从配置文件读取对应属性文件acp_sdk.properties中的 acpsdk.singleQueryUrl
			...
			if(AcpService.validate(rspData, DemoBase.encoding)){
				...
				if(("00").equals(rspData.get("respCode"))){//如果查询交易成功
					...
					if(("00").equals(origRespCode)){
						...
						//订单处理中或交易状态未明，需稍后发起银行卡认证（前台）交易 【如果最终尚未确定交易是否成功请以对账文件为准】
						//TODO
					}else{
						//其他应答码为交易失败
						//TODO
					...
					//订单不存在，可认为交易状态未明，需要稍后发起银行卡认证（前台），或依据对账结果为准
		...
		resp.getWriter().write("银行卡认证（前台）交易</br>请求报文:<br/>"+reqMessage+"<br/>" + "应答报文:</br>"+rspMessage+"");
